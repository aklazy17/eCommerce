...
{
    string databaseName = builder.Configuration.GetConnectionString("ProductDbConnection") ?? throw new InvalidOperationException("ProductDbConnection is not set");
...
builder.Services.AddHttpClient("ProductDetail", u =>
{
    var url = builder.Configuration["ServiceUrls:ProductDetailEndpoint"] ?? throw new InvalidOperationException("Product detail endpoint is not set");
    u.BaseAddress = new Uri(url);
}).AddServiceDiscovery();
